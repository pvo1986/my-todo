...
function TodayView({ data, setData }) {
  ...
  return (
    <div className="view today-view">
      ...
      {(editingTask || newTask) && (
        <TaskModal
          task={editingTask || { id: crypto.randomUUID(), title: "", listId: lists[0]?.id || "", startDate: todayStr, recurrence: { type: "once" }, rollover: false, note: "" }}
          lists={lists}
          onSave={saveTask}
          onDelete={deleteTask}
...
function CalendarView({ data, setData }) {
  ...
  return (
    <div className="view calendar-view">
      ...
      {(newTask || editingTask) && (
        <TaskModal
          task={editingTask || { id: crypto.randomUUID(), title: "", listId: lists[0]?.id || "", startDate: selectedDay, recurrence: { type: "once" }, rollover: false, note: "" }}
          lists={lists}
          onSave={saveTask}
          onDelete={deleteTask}
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .today-right { display: flex; flex-direction: column; align-items: flex-end; gap: 10px; }
